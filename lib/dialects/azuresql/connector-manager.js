...
module.exports = (function() {
  var ConnectorManager = function(sequelize, config) {
    try {
      if (config.dialectModulePath) {
        azuresql = require(config.dialectModulePath)
      } else {
        azuresql = require('msnodesql')
      }
    } catch (err) {
      console.log('You need to install azuresql package manually')
    }
  ...
  var connect = function(done, config) {
	...
	var connectionString = config.username + ":" + config.password + "@" + config.host + config.database;
	console.log("AzureSQL connection string: " + connectionString);

	
	var connection = azuresql.open(connectionString,function(err,conn){
		if (err) {
		        console.log("Error opening the AzureSQL connection!");
				done(err);
		    }
		
			console.log('AzureSQL connection success');		
